...
print(
    "The SciPy version is {0}\n".format(scipy.version.full_version)
...
def extract_features(X, val_X, test_X, max_ngram_size=2):
    ...
    vect = sklearn.feature_extraction.text.CountVectorizer(
        ngram_range=(1, max_ngram_size), tokenizer=TreebankWordTokenizer().tokenize
    )
    ...
    np.savez(
        "converted/X_features-" + str(max_ngram_size) + ".npz", features=X_features
    )
    np.savez(
        "converted/val_X_features-" + str(max_ngram_size) + ".npz",
        features=val_X_features,
    )
    np.savez(
        "converted/test_X_features-" + str(max_ngram_size) + ".npz",
        features=test_X_features,
    )
...
def main():
    ...
    parser.add_argument(
        "--data", help="The input hdf5 file", type=str, default="data_no_batch.h5"
    )
    ...
    with h5py.File(args.data, "r") as f:
        if not os.path.isfile(
            ...
            X_features, val_X_features, test_X_features = extract_features(
                train_x, valid_x, test_x, args.ngram
            )
        else:
        ...
        for index, condition in enumerate(conditions):
            ...
            #    s = subset/float(20)
            acc, auc = make_predictions(
                X_features, train_y, val_X_features, valid_y, test_X_features, test_y, 1
            )  # s)
