...
def main():
    ...
    parser.add_argument(
        "--padding", help="padding around each text", type=int, default=4
    )
    parser.add_argument(
        "--batchsize",
        help="batchsize if you want to batch the data",
        type=int,
        default=1,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--filename", help="File name for output file", type=str, default="data.h5"
    )
    ...
    inputs, targets, ids, subj, time = parse_input_csv(
        args.source, textfield, conditions, id_field, subj_field, chart_field
    )
    ...
    if args.batchsize > 1:
        ...
        size = int(blocks * args.batchsize)
        print("using {} data points".format(size))
        lbl = lbl[:size]
        targets = targets[:size]

        train_size = int(round(blocks * 0.7) * args.batchsize)
        val_size = int(round(blocks * 0.1) * args.batchsize)
        test_size = size - train_size - val_size
        print("using {} train and {} validation examples".format(train_size, val_size))

        xtrain, xval, ytrain, yval = train_test_split(
            lbl, targets, train_size=train_size
        )
        ...
        with h5py.File(filename, "w") as f:
            ...
            f["train"] = np.zeros((int(train_size / b), b, max_len_sent), dtype=int)
            f["train_label"] = np.zeros(
                (int(train_size / b), b, len(conditions)), dtype=int
            )
            f["test"] = np.zeros((int(test_size / b), b, max_len_sent), dtype=int)
            f["test_label"] = np.zeros(
                (int(test_size / b), b, len(conditions)), dtype=int
            )
            f["val"] = np.zeros((int(val_size / b), b, max_len_sent), dtype=int)
            f["val_label"] = np.zeros(
                (int(val_size / b), b, len(conditions)), dtype=int
            )
            # all the IDs
            f["train_id"] = np.zeros((int(train_size / b), b), dtype=int)
            f["test_id"] = np.zeros((int(train_size / b), b), dtype=int)
            f["val_id"] = np.zeros((int(train_size / b), b), dtype=int)
            f["train_subj"] = np.zeros((int(train_size / b), b), dtype=int)
            f["test_subj"] = np.zeros((int(train_size / b), b), dtype=int)
            f["val_subj"] = np.zeros((int(train_size / b), b), dtype=int)
            f["train_time"] = np.zeros((int(train_size / b), b), dtype=int)
            f["test_time"] = np.zeros((int(train_size / b), b), dtype=int)
            f["val_time"] = np.zeros((int(train_size / b), b), dtype=int)
            ...
            for batch in range(b):
                for row in range(int(train_size / b)):
        ...
        xtrain, xval, ytrain, yval = train_test_split(lbl, targets, train_size=0.7)
        xval, xtest, yval, ytest = train_test_split(xval, yval, train_size=0.33)
